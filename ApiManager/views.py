...
from ApiManager.models import ProjectInfo, ModuleInfo, TestCaseInfo, EnvInfo, TestReports, DebugTalk, \
    TestSuite
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if UserInfo.objects.filter(username__exact=username).filter(password__exact=password).count() == 1:
            ...
            return render_to_response("api/login.html")
...
def index(request):
    ...
    return render_to_response('api/index.html', manage_info)
...
def add_project(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(get_ajax_msg(msg, '/http/api/project_list/1/'))

    elif request.method == 'GET':
        manage_info = {
            'account': account
        }
        return render_to_response('api/add_project.html', manage_info)
...
def add_module(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(get_ajax_msg(msg, '/http/api/module_list/1/'))
    elif request.method == 'GET':
        manage_info = {
            'account': account,
            'data': ProjectInfo.objects.all().values('project_name')
        }
        return render_to_response('api/add_module.html', manage_info)
...
def add_case(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(get_ajax_msg(msg, '/http/api/test_list/1/'))
    elif request.method == 'GET':
        manage_info = {
            'account': account,
            'project': ProjectInfo.objects.all().values('project_name').order_by('-create_time')
        }
        return render_to_response('api/add_case.html', manage_info)
...
def add_config(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(get_ajax_msg(msg, '/http/api/config_list/1/'))
    elif request.method == 'GET':
        manage_info = {
            'account': account,
            'project': ProjectInfo.objects.all().values('project_name').order_by('-create_time')
        }
        return render_to_response('api/add_config.html', manage_info)
...
def run_test(request):
    ...
    if request.is_ajax():
        ...
        return render_to_response('api/report_template.html', runner.summary)
...
def project_list(request, id):
    ...
    if request.is_ajax():
        ...
        pro_list = get_pager_info(
            ProjectInfo, filter_query, '/http/api/project_list/', id)
        manage_info = {
            'account': account,
            'project': pro_list[1],
        ...
        return render_to_response('api/project_list.html', manage_info)
...
def module_list(request, id):
    ...
    if request.is_ajax():
        ...
        module_list = get_pager_info(
            ModuleInfo, filter_query, '/http/api/module_list/', id)
        manage_info = {
            'account': account,
        ...
        return render_to_response('api/module_list.html', manage_info)
...
def test_list(request, id):
    ...
    if request.is_ajax():
        ...
        test_list = get_pager_info(
            TestCaseInfo, filter_query, '/http/api/test_list/', id)
        manage_info = {
            'account': account,
            'test': test_list[1],
        ...
        return render_to_response('api/test_list.html', manage_info)
...
def config_list(request, id):
    ...
    if request.is_ajax():
        ...
        filter_query = set_filter_session(request)
        test_list = get_pager_info(
            TestCaseInfo, filter_query, '/http/api/config_list/', id)
        manage_info = {
            'account': account,
            'test': test_list[1],
            'page_list': test_list[0],
        ...
        return render_to_response('api/config_list.html', manage_info)
...
def edit_case(request, id=None):
    ...
    return render_to_response('api/edit_case.html', manage_info)
...
def edit_config(request, id=None):
    ...
    return render_to_response('api/edit_config.html', manage_info)
...
def env_set(request):
    ...
    if request.is_ajax():
        ...
        return render_to_response('api/env_list.html', {'account': account})
...
def env_list(request, id):
    ...
    if request.method == 'GET':
        env_lists = get_pager_info(
            EnvInfo, None, '/http/api/env_list/', id)
        manage_info = {
            'account': account,
            'env': env_lists[1],
            'page_list': env_lists[0],
        }
        return render_to_response('api/env_list.html', manage_info)
...
def report_list(request, id):
    ...
    if request.is_ajax():
        ...
        report_list = get_pager_info(
            TestReports, filter_query, '/http/api/report_list/', id)
        manage_info = {
            'account': request.session["now_account"],
        ...
        return render_to_response('api/report_list.html', manage_info)
...
def view_report(request, id):
    ...
    return render_to_response('api/view_report.html', {"reports": mark_safe(reports)})
...
def periodictask(request, id):
    ...
    if request.is_ajax():
        ...
        task_list = get_pager_info(
            PeriodicTask, filter_query, '/http/api/periodictask/', id)
        manage_info = {
            'account': account,
            'task': task_list[1],
    ...
    return render_to_response('api/periodictask_list.html', manage_info)
...
def add_task(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(get_ajax_msg(msg, '/http/api/periodictask/1/'))
    elif request.method == 'GET':
        info = {
            'account': account,
            'env': EnvInfo.objects.all().order_by('-create_time'),
        ...
        return render_to_response('api/add_task.html', info)
...
def upload_file(request):
    ...
    if request.method == 'POST':
        ...
        return JsonResponse({'status': '/http/api/test_list/1/'})
...
def debugtalk(request, id=None):
    if request.method == 'GET':
        ...
        obj.debugtalk = code
        obj.save()
        return HttpResponseRedirect('/http/api/debugtalk_list/1/')
...
def debugtalk_list(request, id):
    ...
    debugtalk = get_pager_info(
        DebugTalk, None, '/http/api/debugtalk_list/', id)
    manage_info = {
        'account': account,
        'debugtalk': debugtalk[1],
        'page_list': debugtalk[0],
    }
    return render_to_response('api/debugtalk_list.html', manage_info)
...
def suite_list(request, id):
    ...
    if request.is_ajax():
        ...
        filter_query = set_filter_session(request)
        pro_list = get_pager_info(
            TestSuite, filter_query, '/http/api/suite_list/', id)
        manage_info = {
            'account': account,
            'suite': pro_list[1],
            'page_list': pro_list[0],
        ...
        return render_to_response('api/suite_list.html', manage_info)
...
def add_suite(request):
    ...
    if request.is_ajax():
        ...
        return HttpResponse(get_ajax_msg(msg, '/http/api/suite_list/1/'))
        ...
        return render_to_response('api/add_suite.html', manage_info)
...
def edit_suite(request, id=None):
    ...
    return render_to_response('api/edit_suite.html', manage_info)
...
def echo(request):
    if not request.is_websocket():
        return render_to_response('api/echo.html')
    else:
        servers = []
